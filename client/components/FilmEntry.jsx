...
var FilmEntry = ({handleFilmClick, film}) => (
  <div className="film-entry" onClick={() => handleFilmClick(film)}>
    ...
    <div className="film-entry-info">
      <div className="film-entry-name">{film.name}</div>
      <div className="film-entry-genre">Genre: {film.genre}</div>
      <Rating className="ratingStar" empty="fa fa-star-o fa-2x" full="fa fa-star fa-2x" initialRate={film.rating} readonly={true}/>
    </div>
  </div>
)
